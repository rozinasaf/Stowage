...
REGISTER_ALGORITHM(_307881359_a)

int _307881359_a::getInstructionsForCargo(const std::string& input_file_name, const std::string& output_file_name) {
...
int _307881359_a::readShipPlan(const std::string& file_name) {
	s = std::make_shared<Ship>();
	return s->readShipPlan(file_name);
}
int _307881359_a::readShipRoute(const std::string& file_name_rout) {
...
std::vector<int> _307881359_a::getEmptyIndex(int notI, int notJ) {
	int minIndexVal = INT_MAX, minIndexX = -1, minIndexY = -1;
	std::vector<int> res;
	for (int i = 0; i < s->getxMax(); i++)
		for (int j = 0; j < s->getyMax(); j++)
			if (i != notI && j != notJ && s->getShipPlanNow(i, j) < s->getShipPlanMax(i, j) && s->getShipPlanNow(i, j) < minIndexVal) {
				minIndexX = i;
				minIndexY = j;
				minIndexVal = s->getShipPlanNow(i, j);
			}
	if (minIndexVal == INT_MAX)
		return { -1,-1,-1 };
	else
		return { minIndexX,minIndexY,s->getShipPlanNow(minIndexX, minIndexY) + s->getShipPlanOccupied(minIndexX, minIndexY) };
}


void _307881359_a::tryMoveAllAbove(int i, int j, int k, int n) {
...
void _307881359_a::unloadAtPort(const std::string& port, int n) {
...
std::vector<std::vector<std::string>> _307881359_a::getContainersFromCloseToFar(int currentStopIndex, const std::string& travel_port_file_name) {
	std::string dest;
...
int _307881359_a::tryLoading(const std::string& travel_port_file_name, const std::string& container, int stopIndex) {
	int errors = 0;
	...
	else if (lineError != 0 && isNewError(errors, 1 << 14))
		errors = 1 << 14;
	return errors;
}
int _307881359_a::operate(int i, const std::string& travel_port_file_name) {
	int errors = 0;
...
if (lineError != 1 << 14) // IF THERE IS NO IGNORE ERROR
		...
		for (int j = 0; j < (int)toLoad.size(); j++) {
			...
			if (loadErrors != 0 && isNewError(errors, loadErrors)) errors += loadErrors;
		}
	}
